...
主题赛道与编辑层级彼此独立：TDE、太阳耀斑、任务方法和多信使宇宙学等方向均可因直接服务团队目标而进入核心层；层级由直接性判断，优先级分数只负责层内排序。每日页不再复述篇数与固定主题分布，而是严格判断是否存在可由具体论文支撑的 `BREAKING` / `HEADLINE`，无充分证据时明确显示“今日无突发头条”。
