...
主题赛道与编辑层级彼此独立：TDE、太阳耀斑、任务方法和多信使宇宙学等方向均可因直接服务团队目标而进入核心层；层级由直接性判断，优先级分数只负责层内排序。每日页不再复述篇数与固定主题分布，而是经两轮编辑复核筛选可由具体论文原题、原摘要支撑的 `BREAKING` / `HEADLINE`。每篇论文独立生成一个短 JSON 决策，只有入围论文才逐篇进入第二轮，最终由程序校验、合并和去重；不存在整日大 JSON。新闻简报、评论、更正等二手条目不能借用其报道对象的重要性入选。新闻不设主头条、不设数量名额，同一等级平级展示；无充分证据时明确显示“今日无重大头条”。
